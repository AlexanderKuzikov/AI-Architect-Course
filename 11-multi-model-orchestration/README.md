...
8. [Реальный кейс: RouterAI как fallback pool](#8-реальный-кейс-routerai-как-fallback-pool)
...
│ CLOUD_REASONING_MODEL (alt vendor)  │ 60   │ 1 000 │ 300K    │
...
Telegram posts folder
  ...
  └──────┬──────┘
         │
    yes (~30%)    ← SLM gate: 30% изображений проходят в облачную VLM
         │
         ▼
  ┌─────────────┐
...
> **Статистика фильтрации:** SLM gate (0.8B, локально) пропускает ~30% изображений
> в облачную VLM. Остальные 70% обрабатываются pytesseract + локальной text LLM —
> в 5× быстрее и дешевле при сопоставимом качестве на чистых сканах.
> Итог: cost/день снижается в ~2.5× относительно «всё через облако».
...
## 8. Реальный кейс: RouterAI как fallback pool

### Контекст

**Задача:** PDFtoText — сервис обработки PDF для VLM-пайплайна. Основной маршрут — локальная модель, но часть документов (сложные сканы, нестандартные формы) требуют облачного VLM. Прямой доступ к OpenAI из РФ нестабилен, оплата в валюте. Решение: RouterAI (routerai.ru) — российский роутер LLM API, OpenAI-совместимый, оплата в рублях (~3₽/1M входных токенов).

**Почему это fallback, а не основной маршрут:** локальный VLM/OCR решает простые случаи (70–80% документов), RouterAI — сложные сканы низкого качества. Облако в цепочке только там, где локальная модель не дотягивает.
...
PDF → PDFium/OCR
        │
        ▼
  локальный VLM (LM Studio)
        │
   pass (сложный скан)     ← локальная модель не дала результат
        │
        ▼
  RouterAI /chat/completions (qwen/qwen3.7-flash, ₽-оплата)
        │
        ▼
  outputNormalizer → structured data
```

Переход на RouterAI — по сигналу качества, а не по ошибке сети: локальная модель вернула мусор (PUA-символы, OCR-артефакты) или confidence ниже порога. Это та же логика, что в §3, только fallback — платный облачный роутер.

### Грабли, которые всплыли в production

**1. `error` в ответе — строка, не объект.** Обработчик парсил `err.message` как объект и падал с TypeError на самой ошибке. Нужен парсер обоих вариантов: string или `{"message": ...}`.

**2. Сломанный PDF → HTTP 200.** RouterAI принимает PDF напрямую как `type:"file"` с base64. Битый PDF не даёт 4xx: плагин file-parser вставляет в контент маркер `<file name="..." status="error">сообщение</file>`, и модель «отвечает» на него. Без детекта маркера мусорный текст проходит весь pipeline как успешный результат — та же ошибка, что false negative на gate в §4, но в обёртке успешного HTTP.

**3. `</think>`-обрывки в content у VLM.** qwen3.7-flash через vision возвращает хвост рассуждений даже при `reasoning.effort: none`. Ломает JSON-парсинг structured output. Решение — тот же `outputNormalizer`, что и для `CURRENT_TEXT_MODEL mid` в §3: вырезать думание до парсинга.

**4. Умная маршрутизация внутри роутера.** RouterAI маршрутизирует запросы между провайдерами по цене: запросы с файлами и без файлов могут уходить разным провайдерам, и валидация параметров (`effort`) у них отличается. Model Rotation «внутри» одного ключа: поведение меняется без смены baseURL и ключа — самое неочевидное место при отладке.

**5. Дёшево проверить ключ — «ping».** Перед батчем ключ проверяется chat-запросом `"ping"` с `temperature: 0` и малым `max_tokens`. Стоимость пренебрежима, зато сломанный ключ и 401/403 не роняют середину пайплайна из тысячи документов.

### Вывод

Fallback-провайдер — не «другая модель в try/catch». Каждый шаг цепочки несёт собственные грабли: формат ошибок, ложные HTTP 200, обрывки reasoning, скрытую маршрутизацию. Нормализация вывода (§3), детект маркеров ошибок и дешёвый health-check ключа — обязательные элементы интеграции, а не «допилю потом».

**Практический вывод для архитектора:** роутер-провайдер (RouterAI, OpenRouter и т.п.) — не прозрачный прокси, а ещё один слой с собственным поведением. Внутри одного ключа могут ротироваться провайдеры: при отладке деградации качества метрика «модель» должна дополняться метрикой «провайдер», иначе половина отклонений необъяснима.

### Дополнение из практики: связки моделей для ревью (сентябрь 2026)

Одиночная модель пропускает темы. В стенде слепых ревью лучшая пара закрыла двадцать две темы из двадцати четырёх, тройка — двадцать три. У одной модели уникальных тем не было вовсе, а одну редкую находку с состоянием гонки поймала только шестая модель — пропустили десять прогонов остальных. Правило отсюда: для поиска багов берут веер из двух-трёх разных моделей плюс страховку, для рутины — поток через одну, для спорного — ворота с арбитражем. Связки пересчитывают при выбывании модели, а не хранят как константу.
