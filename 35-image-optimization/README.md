...
## 7. Реальный кейс: Sharp-пайплайн в VLM-конвейере — экономика размера

### Контекст

PDFtoText: документы → рендер (PDFium WASM) → изображения страниц → VLM (локальная и облачная). Фото/сканы приходят в исходном размере: ценники 3000×4000px, документы A4@300dpi — 8.7 МП (модуль 16: VLM может не принять без resize).

### Задача

Не отправлять в модель исходные размеры: облачные VLM тарифицируют по tile-системе (OpenAI: 512×512 tiles — изображение 3000×4000 = 24 tiles = 24× стоимость против 1000px), локальные модели обрезают изображение токенизатором при превышении vision budget (модуль 11 §5).

### Гипотеза

Единый модуль оптимизации перед каждым VLM-вызовом: resize по задаче (1000px для ценников/текста, 1536px для плотных документов), WebP quality 85, `fit: inside` + `withoutEnlargement` (модуль 11 §5).

### Что получилось

```typescript
// Реальный модуль из pipeline (модуль 11 §5)
...
// Ценники/короткий текст: 1000px, WebP 85
// Документы с мелким шрифтом: 1536px (только если 1000px даёт деградацию)
```

- Ценники 1000px вместо 3000px — tile-стоимость облачной VLM падает в разы при том же качестве детекции (проверено на выборке, модуль 21 — golden-подход);
- рендер PDFium → Sharp: `raw: { channels: 4 }` — RGBA-каналы (модуль 16 §9): без явных каналов Sharp интерпретирует bitmap неправильно;
- правила размера из модуля 11 §5: 1000px — дефолт, 1536px — только плотный текст, выше — не отправлять.

### Грабли, найденные в production

1. **`withoutEnlargement` забыли** — маленькие изображения (иконки в документах) «растягивались» до 1000px: мыло на выходе VLM. Опция стоит по умолчанию.
2. **CMYK/профили из сканеров** — Sharp по умолчанию не конвертит профиль: зелёные оттенки на отрендеренных страницах. Конвертация в sRGB — часть пайплайна.
3. **Качество 85 для облака, 80 для локальной VLM** — облачная VLM платит за токены (важнее размер), локальная — за качество распознавания мелкого текста. Один параметр `quality` на вызов, не на систему.
4. **Resize до VLM, а не после**: сначала Sharp, потом base64 в запрос — иначе в облако уходит неоптимизированный оригинал.

### Вывод, противоречащий интуиции

Оптимизация изображений в VLM-конвейере — это **экономика вызова, а не «сжать картинку»**: resize перед отправкой напрямую режет tile-стоимость облачной VLM и спасает локальные модели от обрезки токенизатором. Один модуль с явными параметрами (maxDimension/quality/format/fit) работает на весь конвейер; правка в одном месте меняет все VLM-вызовы.

**Практический вывод для архитектора:** изображения — пограничный ресурс между системами (VLM-тарификация, vision budget, качество распознавания). Единый модуль оптимизации с явными параметрами — инвестиция, окупающаяся на каждом вызове модели: и в деньгах (tiles), и в качестве.

### Дополнение из практики: операторский контур с фото (сентябрь 2026)

Тот же Sharp, но со стороны оператора: загрузка фото с телефона, конвертация в WebP, GPS из метаданных первого фото в координаты, вставка двух чисел одной строкой с разбором порядка. Вложения письма и финал заявки фильтруются строго по списку фото из метаданных — иначе за день в письмо налипают orphan-файлы из черновика. Шрифты — локальными файлами с разделением латиницы и кириллицы, без внешних CDN. Правило: файл в папке — не факт заявки, факт — запись в метаданных.
