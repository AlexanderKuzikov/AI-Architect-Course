...
| **Kombinator SaaS** | Шаблон держат юристы в конструкторе, код только шлёт данные | Нужен офлайн или полный контроль (тогда docxtemplater) | 14, 51 |
...
| **systemd timers** | Фоновые задачи на голой VPS: бэкап, health-check, витрина по требованию | Распределённые очереди и DAG (тогда BullMQ/pg-boss) | 24, 25 |
| **SFTP по ключу** | Передача файлов на VPS тем же пользователем | Shared-хостинг с грейлистом FTP (там только через владельца) | 24, 44 |
...
| Инструмент | Когда брать | Когда не брать | Модуль |
|-----------|-------------|----------------|--------|
| **Go + WebView (webview_go)** | Маленький бинарник, HTML-UI, Go-бэкенд | Нужен зрелый экосистемный инструментарий (Electron) | 48 |
| **Wails v2 (Go + WebView2)** | Тонкая оболочка над веб-билдом, десяток мегабайт, системный движок | Нет Go-стека, нужен чистый веб без оболочки | 48 |
| **Electron** | Размер не критичен, нужен zрелый tooling | Размер 150 MB неприемлем | 48 |
| **Tauri** | Rust-стек, system WebView | Нет Rust-стекa | 48 |
| **Fyne** | Простой нативный виджетный UI | Нужен полный HTML/CSS/JS | 48 |

## Frontend-таблицы (выбор полигоном)

| Инструмент | Когда брать | Когда не брать | Модуль |
|-----------|-------------|----------------|--------|
| **React + AG Grid** | Плотные таблицы как в 1С, Excel-copy из коробки, десятки тысяч строк | Лёгкий лендинг без таблиц | 31, 48 |
| **Vue + AG Grid (ветка)** | Команда на Vue, переезд за неделю-две | Ядро с нуля (React предсказуемее для LLM) | 31 |
| **Svelte (лёгкий)** | Минимальный бандл и быстрый старт | Сложный грид — кастом, модели путают синтаксис | 31 |

## SQL / Хранилища (49)

| Инструмент | Когда брать | Когда не брать | Модуль |
|-----------|-------------|----------------|--------|
| **PostgreSQL** | Связи, транзакции, запросы, миллионы строк | Простой read-only корпус | 49 |
...
| Инструмент | Когда брать | Когда не брать | Модуль |
|-----------|-------------|----------------|--------|
| **RouterAI** | Российский рынок, оплата в рублях | Западные клиенты | 50, 19 |
| **RouterAI VLM + file-parser** | PDF файлом без растрирования, текстовые — бесплатно, сканы — платный OCR | Нужен детерминированный локальный OCR без сети | 10, 50 |
| **edge-tts (бесплатный endpoint)** | Озвучка учебных текстов без ключей и VPN, замедление движком | Продакшен TTS со SLA (тогда облачный TTS по контракту) | 48 |
| **OpenRouter** | Ротация ключей, fallback провайдеров | Один надёжный контракт | 50 |
| **Локальная модель (LM Studio/Ollama)** | Приватность, нулевой marginal cost | Нужно качество облачной модели | 50, 08 |
| **Langfuse / cost attribution** | Cost per task, fallback tracking | Прототип | 50, 46 |

## API Design (51)

| Инструмент | Когда брать | Когда не брать | Модуль |
|-----------|-------------|----------------|--------|
| **OpenAPI 3.1 (spec)** | Внешний контракт, контрактные тесты | Внутренний монорепо-сервис | 51 |
| **@redocly/cli** | Линт и валидация спецификации в CI | — | 51 |
| **Плоский JSON** | 1С и кодогенерируемые клиенты | Глубокие вложенные графы | 51 |
| **RFC 9457 Problem Details** | Стандартное тело ошибок | 1С-потребитель (нужен плоский `{code, message, details, request_id}`) | 51 |
| **Idempotency-Key (header)** | Произвольные POST-операции | Импорт по бизнес-ключу (ON CONFLICT) | 51, 49 |

## Resilience Patterns (52)

| Инструмент | Когда брать | Когда не брать | Модуль |
|-----------|-------------|----------------|--------|
| **opossum 8.x** | Circuit breaker в Node.js | Go-стек (свой через каналы) | 52, 19 |
| **Токен-баскет (свой/Redis)** | Общий rate limit на downstream | Один воркер, низкий темп | 52, 23 |
| **Bulkhead (семафоры)** | Разделение потребителей по ресурсам | Нет конкуренции за ресурс | 52 |
| **Saga-таблица (PostgreSQL)** | Состояние распределённых операций | Операция в одной БД | 52 |
| **DLQ (pg-boss)** | Компенсации/задания, исчерпавшие повторы | — | 52, 18 |
