...
## 7. Реальный кейс: критический путь PHP-сайта — от 2.9s до 1.4s FCP

### Контекст

PHP-сайт на собственном фреймворке (WebForge, модули 03/29): страницы собираются в рантайме, общий CSS 40 KB в `<head>`, jQuery-плагины в футере. FCP 2.9s на 4G-эмуляции, LCP 4.1s (hero-изображение).

### Задача

Сократить критический путь: до первого отрисованного пикселя проходит HTML → render-blocking CSS → картинки. Причины по порядку вкладa: render-blocking 40 KB CSS, шрифты с блокировкой, hero как CSS background.

### Гипотеза

Поэтапный фикс по модели CRP: (1) критический CSS инлайн (модуль 29), (2) async-загрузка остального CSS, (3) шрифты `display=optional` + preload, (4) hero из background → `<img fetchpriority="high">`.

### Что получилось

| Шаг | FCP | Комментарий |
|:--|:--|:--|
| Baseline | 2.9s | 40 KB CSS блокирует |
| + Critical CSS inline (7 KB) | 1.9s | render-blocking убран |
| + Async style.css (media print trick) | 1.7s | остальной CSS не блокирует |
| + Шрифты optional + preload woff2 | 1.5s | нет FOIT-блокировки |
| + Hero `<img>` вместо background | 1.4s | preload scanner видит картинку |

Каждый шаг — отдельное измерение (Lighthouse + реальные замеры): видно, что дало больше всего (CSS inline — минус 1s) и что почти ничего (шрифты при отсутствии рендер-блокировки самих шрифтов в современном Chrome).

### Грабли, найденные в production

1. **Порядок фиксов решает**: сначала CSS (главный блокер), потом шрифты, потом картинки. «Оптимизация hero-картинки» без снятия render-blocking CSS дала бы меньше.
2. **Async CSS + FOUC**: media print trick без `<noscript>` — страница «голая» при JS disabled (Googlebot иногда рендерит без JS). noscript-фолбэк обязателен (модуль 29).
3. **Шрифты — не всегда блокер**: в современном Chrome render-blocking шрифтов нет (FOIT-период), поэтому шаг дал мало. Измерение каждого шага отдельно — защита от «оптимизаций по интуиции».

### Вывод, противоречащий интуиции

80% выигрыша дал один фикс — инлайн критического CSS; остальные три шага суммарно — 0.5s. Модель CRP правильно указала приоритет (CSS → шрифты → картинки), но без поэтапного измерения половина работы была бы «оптимизацией того, что не блокировало». Критический путь — это цепочка; фиксы дают выигрыш только в правильном порядке.

**Практический вывод для архитектора:** CRP-оптимизация — последовательный процесс с замером на каждом шаге: убрал блокер → измерил → следующий. Порядок «CSS → шрифты → медиа» из модели CRP, а не «что видно в DevTools первым».
...
Формула: критический CSS ≤ 20KB + async-загрузка остального + проверка FCP + цикл сокращения.
