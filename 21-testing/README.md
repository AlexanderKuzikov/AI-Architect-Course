...
5. [Тестирование в AI-assisted разработке](#5-тестирование-в-ai-assisted-разработке)
6. [Тестирование AI-выводов](#6-тестирование-ai-выводов)
7. [Snapshot-тестирование](#7-snapshot-тестирование)
8. [Contract testing для сервисов](#8-contract-testing-для-сервисов)
9. [Performance testing](#9-performance-testing)
10. [Реальный кейс: тестирование PDF/VLM-пайплайна](#10-реальный-кейс)
...
## 8. Contract testing для сервисов
...
## 10. Реальный кейс: тестирование PDF/VLM-пайплайна

### Контекст

PDFtoText — пайплайн «PDF → текст/рендер → VLM → structured JSON»: ~500 PDF/день (судебные решения, договоры, сканы 1С). Два класса проблем: парсинг документов (битый ToUnicode, Type1-шрифты — модуль 15/16) и недетерминированные AI-выводы (модуль 6/7).

### Задача

Тесты, которые ловят регрессии парсинга И деградацию AI-выводов, не превращаясь в хрупкие «тесты на равенство строк».

### Гипотеза

Три уровня вместо пирамиды юнит-тестов:

1. **Golden dataset** — фиксированная выборка документов с ожидаемыми результатами;
2. **Schema-валидация** — каждый VLM-вывод проверяется на структуру (модуль 7);
3. **Пороговые метрики** — не равенство, а «не хуже N%» на выборке (модуль 9).

### Что получилось

Golden-выборка из 50 PDF стала главным тестом регрессий рендера. Реальное сравнение движков (модуль 16 §10) — это и был golden-тест:
...
- Выборка фиксирована, результаты — часть CI: новый движок/библиотека, давшая артефакты на 2 документах больше, — красный билд;
- PUA-диагностика (>5% PUA-символов → render → VLM) — тестируемое поведение, а не «магия»: golden-тест проверяет, что маршрут выбран правильно для каждого класса документа;
- VLM-выводы: не «строка равна», а schema-валидация + порог `successRate ≥ 0.95` на выборке (алерт `SUCCESS_RATE_DROP` — выше в §9).

### Грабли, найденные в production

1. **Golden-выборка устаревает**: новые классы PDF (новые источники 1С) не попадают в выборку — регрессия проходит тесты. Выборку пополняют инцидентами: «документ, на котором упало в прод-е, обязан стать golden-тестом».
2. **VLM-недетерминизм**: два прогона одной выборки дают разные JSON-ответы. Тест на равенство — флаки; тест на схему + порог — стабилен.
3. **Контрактные тесты для 1С** (модуль 51): CI проверяет, что сервер согласуется со спецификацией — это закрыло класс регрессий «поменяли формат, забыли обновить контракт».

### Вывод, противоречащий интуиции

В пайплайне с AI самые ценные тесты — не юнит-тесты компонентов, а **контракты**: golden-выборка (контракт парсинга), schema (контракт вывода), OpenAPI (контракт API). Юнит-тесты покрывают форс-мажорные ветки, но регрессии, которые реально больно бьют, ловятся контрактами на выборке данных.

**Практический вывод для архитектора:** «тестируемость» AI-пайплайна закладывается данными: фиксированная выборка + схема + пороги. Каждый прод-инцидент обязан пополнять выборку — это превращает тесты из обязанности в растущий актив.

### Дополнение из практики: golden-сверка со старой системой и DoD (сентябрь 2026)

Миграция со старой учётной системы закрывается выгрузкой из неё и дифф-отчётом новой платформы с критерием ноль расхождений: только тогда старую можно отключать. Расчёты покрываются golden-тестами поштучно, деньги — отдельным набором с капами и балансом. Демо-фабрика генерирует правдоподобные сущности со связными именами и сценариями — для демо, разработки и тестов, а не руками собранные три строки.

Definition of Done как тестовая петля: типы зелёные, линтер зелёный, тесты модуля зелёные с новым тестом на изменённую логику, описания затронутых модулей актуальны, запись в журнале контекста. Проверка приёмки — повторением статики принимающей стороной, а не доверием отчёту: типы, полный прогон, линтер, обе сборки. Живые проверки, невыполнимые в среде сессии, в приёмку не пишутся.
...
Формула: Vitest + testcontainers (версии) + сценарии (успех/ошибка/дубль) + без моков DB + изоляция через TRUNCATE.
...
Формула: Playwright + селекторы (getByRole/getByTestId) + сценарии (успех/отказ) + mock API через page.route + retries: 0.

---

## 13. Чеклист архитектора
