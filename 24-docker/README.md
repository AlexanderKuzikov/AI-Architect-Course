...
### Дополнение из практики: голая VPS рядом с Docker (сентябрь 2026)

Не всё живёт в контейнерах. Витрина крутится так: Caddy на хосте слушает восемьдесят и четыреста сорок три и раздаёт TLS, PHP идёт через сокет, а в Docker — только состояние: Postgres, Redis с журналом, объектное хранилище. Проксирует Caddy только порт API, сами базы наружу не торчат.

Что забрать в модуль:

- Домен без покупки и Госуслуг: wildcard-DNS на IP даёт имя сразу, Caddy сам забирает сертификат при первом запросе. Смена IP — смена имени, для статического адреса не проблема.
- Харденинг до первого деплоя: файрвол с дефолтным запретом входящим, только ssh и веб, fail2ban на ssh, автообновления только для безопасности, отдельный пользователь для работ, вход только по ключу.
- Таймеры вместо крона для фонового: бэкап раз в сутки с ротацией, health-check каждые пять минут с перезапуском упавшего сервиса. Логи — в журнал, статус — одной командой.
- Передача файлов: вместо классического FTP — key-based SFTP тем же пользователем, классический FTP на shared-хостинге душится грейлистом и лечится только капчей владельца — это уже не Docker, а выживание на shared.
- Грабли из жизни: конфиг облачного init перебивает харденинг ssh — класть свой конфиг лексикографически раньше; двойные кавычки оболочки исполняют подстановки локально — команды на сервер в одинарных; сокет доступен веб-пользователю через группу, менять не надо.
