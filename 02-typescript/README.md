...
8. [TypeScript и AI-кодер](#8-typescript-и-ai-кодер)
9. [Задачи AI-кодеру](#9-задачи-ai-кодеру)
10. [Чеклист архитектора](#10-чеклист-архитектора)
...
### Актуальная базовая конфигурация (TypeScript 6.0.3)
...
### Ловушка 6 — мажорный компилятор без экосистемы линтера

Новый мажорный TypeScript может выйти без compiler API: пакет экспортирует только версию, а плагины линтера жёстко требуют предыдущий мажор и падают на импорте. Резолв пиров через переопределения не сходится — пиры схлопываются на корневой версии. Выход — отдельный тулчейн-пакет в том же монорепо: линтер со своей зафиксированной версией компилятора, конфиг один на корень, запуск из корня, компилятор проекта не тронут. Триггер возврата — поддержка нового мажора самим линтером, тогда тулчейн сносится. Правило: версия компилятора и версия тулинга — разные решения с разными циклами жизни.

### Ловушка 7 — импорт мимо публичного входа модуля

В монорепо с жёсткими границами чужой пакет импортируется только через его публичный вход, прямые запросы к чужим таблицам запрещены — только через API владельца. Нарушение выглядит безобидно в малом репо, а лечится потом запретом на уровне ревью: главный файл раздувается до тысяч строк, состояние всех сущностей живёт в одном компоненте. Проверенный путь декомпозиции — выносить по одной сущности за раз в свой модуль с зеркалами состояния, снимком, валидацией и сохранением по идентификатору, App сводится к роутеру-вызовов без имён чужого состояния. Каждая сущность — проверяемо тем же набором: типы и сборка зелёные, поведение один в один, данных не запачкано.
...
- **`any` как способ закончить задачу быстрее.** Это не типизация, а отложенный runtime-баг. Для внешних данных — `unknown` + guard/zod; для внутренних контрактов — точные типы.
- **Utility types ради красоты.** Слишком глубокие mapped/conditional types становятся отдельным языком, который AI и разработчики боятся менять. Сложность должна окупаться снижением ошибок.
- **Type assertion вместо runtime guard.** `as User` ничего не проверяет. На границе API, файла, очереди или промпта нужен парсинг и валидация.
- **Enum для открытых доменов.** Enum удобен, но плохо переживает внешние API, миграции и сериализацию. Для открытых наборов — union literal types или external contract.
- **Типы без инвариантов.** `interface` может описывать форму, но не смысл. Для денег, дат, slug, document id и статусов нужны branded types или value objects.

## 8. TypeScript и AI-кодер
...
- Переписать опасные `any` на `unknown` + validation.
- Добавить branded types для идентификаторов и денежных значений.
- Проверить `tsconfig` на `strict`, `noUncheckedIndexedAccess`, `exactOptionalPropertyTypes`.

## 10. Чеклист архитектора
