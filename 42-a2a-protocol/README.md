...
### Дополнение из практики: штаб и кодовые сессии (сентябрь 2026)

Та же оркестрация, но для людей и сессий: одна штаб-сессия планирует и принимает, по одной кодовой на задачу. Задание — файлом с номером: цель, что входит и не входит, старт-указатели, приёмка командами и живыми проверками, ограничения. Жизненный цикл — открыта, в работе, на проверке, принята. Сессия правит только свой файл и пишет отчёт в него же, диспетчер и архив — только штаб. Объём заморожен: слово штаба посреди сессии объём не меняет, новая работа — новым номером. Главное правило — одна работа на одно задание: если из задания можно выкинуть пункт и остальное сдаётся, это были разные задания. Параллельность — только при своём репозитории на каждую, иначе строго последовательно: общие дев-серверы, порты и база не терпят гонок.

### Дополнение из практики: оркестрация как код и BPMN как язык ТЗ (сентябрь 2026)

Если оркестратор когда-либо понадобится — только оркестрация как код с детерминированными workflow и тестами со сдвигом времени, а не модельные движки, чья семантика разложена между XML, строковыми корреляциями и рантаймом. Та же логика для бизнес-процессов: нотация схем — только как формальный язык техзадания, не как движок исполнения. Этап формализации берут один в один, этап развёртывания на low-code платформе отбрасывают: исполнение остаётся в коде монолита — машина статусов плюс исходящий журнал в очередь.

Хранение схем — в git: исходник плюс копия для демо, версионированиеdiff как подпись «согласовано», чего нет на схеме — того нет в ТЗ. Диалект из пяти правил: дорожка — роль, задача — глагол с документами и сроком, развилка — вопрос с подписанными выходами, таймер — число дней из регламента, объект данных — шаблон документа. Прямая подача в модель: разбор XML в промпт вида развилка в ветвление, задача в шаг, таймер в отложенную задачу, таблица решений в таблицу — дальше схемы, политики и тесты без пересказа. Смотрят схемы в десктоп-моделлере, веб-просмотр для согласования признан неудобным.
