...
## 7. Реальный кейс: critical CSS без инструментов для PHP-сайта

### Контекст

PHP-сайт на собственном мини-фреймворке (нативные шаблоны через `ob_start()`/`ob_get_clean()`, модуль 03): главная, каталог, статьи. Общий `style.css` — ~40 KB, подключён в `<head>` на всех страницах.

### Задача

Render-blocking 40 KB CSS задерживает FCP/LCP на каждой странице. Инструменты экстракции (Beasties/critters) рассчитаны на SSG-пайплайны (постобработка HTML) — здесь сборки нет, страницы собираются в PHP-рантайме.

### Гипотеза

Для небольшого сайта с 3 типами страниц ручная экстракция critical CSS — проще и надёжнее инструмента: контракт «что входит в above-fold» держится в одном шаблоне, а не в конфиге экстрактора.

### Что получилось

```php
<?php // Шаблон head.php — critical CSS инлайн, остальное асинхронно ?>
<style>
  /* :root-переменные и critical-стили (hero, nav, карточки первого экрана) */
  <?= readfile(__DIR__ . '/critical.css'); ?>   <!-- ~7 KB -->
</style>
<link rel="stylesheet" href="/assets/style.css" media="print" onload="this.media='all'">
<noscript><link rel="stylesheet" href="/assets/style.css"></noscript>
```

- `critical.css` — 7 KB: мобильные стили каркаса (mobile-first), навигация, hero, карточки; без них страница «голая»;
- `style.css` грузится асинхронно, из кеша на повторных визитах;
- FCP сократился на время скачивания/парсинга 40 KB CSS; LCP-элемент (hero) отрисован из инлайна.

### Грабли, найденные в production

1. **`:root`-переменные — в critical, иначе CLS**: тема (цвета, отступы) определялась переменными из `style.css`; пока он грузился асинхронно, инлайн-стили ссылались на неопределённые переменные. Переменные переехали в critical.css.
2. **Media-запросы внутри critical**: mobile-first шаблон — critical.css писался под desktop-взгляд, мобильный first paint ломался. Контракт: critical.css пишется от мобильного к desktop.
3. **Обновление critical.css забывали при правках layout** — «страница сломана на первом экране» при живом style.css. Фикс: проверка FOUC вручную на каждой правке шаблона (инструментальной проверки нет — цена ручного подхода).

### Вывод, противоречащий интуиции

Ручная экстракция критического CSS оказалась надёжнее инструментальной для PHP-сайта: инструмент — это постобработка собранного HTML, а здесь нет сборки. Цена — ручная дисциплина (что в critical, mobile-first, переменные); выгода — ноль зависимостей и полный контроль над контрактом выше fold. Для SSG с сотнями шаблонов ручной подход не масштабируется — там Beasties.

**Практический вывод для архитектора:** critical CSS — это контракт «что должен видеть пользователь до полной загрузки CSS». Для 3–5 типов страниц контракт держится руками; для SSG/динамических шаблонов — инструментом. Неважно как извлечено — важно, что контракт включает переменные, mobile-first и обновляется вместе с layout.
...
Формула: media print trick + noscript fallback + preconnect/display=optional для Google Fonts + preload woff2 + границы правок.
