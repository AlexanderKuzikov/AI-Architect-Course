...
### Дополнение из практики: подключение сайта как часть пайплайна (сентябрь 2026)

Диагностика начинается с подключения, а подключение — наполовину ручное. Граница: токен умеет пользователя, хосты, sitemap, переобход и диагностику, проверка живости — запросом пользователя. Не умеет: добавить хост и управлять счётчиком. Порядок: счётчик — кликами с включённым вебвизором, хост — вводом голого домена с подтверждением главного зеркала, подтверждение — HTML-файлом в корне с коммитом в репозиторий, затем по API — sitemap и переобход главной, тумблер обхода по счётчикам — только в интерфейсе вебмастера. После подтверждения нормальны пустые данные первые сутки — повторять baseline, а не чинить.
