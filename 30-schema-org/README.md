...
## 7. Реальный кейс: Product schema для каталога на 50k товаров

### Контекст

Каталог на WooCommerce, ~50 000 товаров, прайсы приходят из 1С (модуль 17: XLSX-выгрузки, нормализация цен «1 234,56»). Задача — Product-разметка на каждой товарной странице для rich results.

### Задача

JSON-LD на 50k страниц: руками не напишешь, шаблон-копипаст с главной — антипаттерн №2. Разметка должна генерироваться из тех же данных, что и видимая страница (антипаттерн №1: разметка ≠ контент).

### Гипотеза

Программная генерация в шаблоне товара из данных БД (единый источник) + `@id` для каждой сущности + автоматическая валидация JSON-LD при сборке.

### Что получилось

```php
<?php // single-product.php — JSON-LD из данных товара, а не из конфига ?>
<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "Product",
  "@id": "<?= esc_js($product->url) ?>#product",
  "name": <?= json_encode($product->name, JSON_UNESCAPED_UNICODE) ?>,
  "sku": <?= json_encode($product->sku) ?>,
  "offers": {
    "@type": "Offer",
    "price": "<?= $product->priceCents / 100 ?>",   // из прайса 1С, канон "1234.56"
    "priceCurrency": "RUB",
...
- Генерация из строки БД (цена уже нормализована из 1С, модуль 17) — разметка не может разойтись со страницей;
- `sku` — из 1С (артикул — бизнес-ключ каталога);
- валидация: скрипт `validateSchemaMarkup` из §6 прогнан по всем страницам при сборке — битый JSON-LD = fail.

### Грабли, найденные в production

1. **Цена из 1С в разметке**: «1 234,56» в `price` — Google не парсит. Канон — строка `"1234.56"` без разделителей. Нормализация на входе (модуль 17) спасла и прайс-листы, и разметку.
2. **`aggregateRating` захотелось без отзывов** — антипаттерн №6: Google может наказать. Оставлен только у товаров с реальными отзывами WooCommerce (count > 0).
3. **`@id` и дубли**: без `@id` каждый рендер создавал «новую» сущность товара — граф не связывал страницы каталога. `@id` = URL товара → единая сущность.

### Вывод, противоречащий интуиции

Разметка на 50k страниц оказалась **проблемой контракта данных**, а не SEO: правильная генерация потребовала нормализации цен из 1С (модуль 17), единого источника (БД, не шаблоны) и валидации в сборке. SEO-разметка, которая «генерируется из видимого контента», не может разойтись с контентом — это единственная надёжная стратегия на масштабе.

**Практический вывод для архитектора:** structured data — это контракт между данными и поисковиком: единый источник, канонические форматы (цены, даты), `@id`, валидация в CI. На масштабе ручная разметка = расслоение контента и JSON-LD, которое Google наказывает.
