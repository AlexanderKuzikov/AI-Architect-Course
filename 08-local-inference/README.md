...
### Cloud-fallback и управление думанием (сентябрь 2026)

Локальный инференс не отменяет облачный fallback для российского рынка: рублёвый роутер с OpenAI-совместимым endpoint закрывает всплески и VLM, которых нет локально. У роутера думание управляется картой effort от выключенного до максимального, выключенное даёт ноль токенов думания и разницу в десятки раз по цене vision-страницы. Параметр провайдеро-зависим и может отличаться между маршрутами внутри одного ключа — проверять по фактической ошибке валидации, а не по доке.

Практика из операторского контура: у одного провайдера думание не отключалось ни флагом, ни подсказкой в промпте, сработал только параметр шаблона чата; токены упали в разы, а латентность осталась под полторы минуты — провайдер заменили на роутер с ответом за секунды и долями копейки. Вывод: параметр думания входит в cost model отдельной строкой, health-check ключа — дешёвым ping-запросом до батча.
