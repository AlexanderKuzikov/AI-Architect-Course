...
## 7. Реальный кейс: lazy loading каталога на 50k товаров

### Контекст

Каталог WooCommerce (модули 30–32): 50k товаров, сетка карточек с изображениями, бесконечная подгрузка. Каталог 1С-прайса без lazy — 50k запросов изображений при первом открытии каталога.

### Задача

Грузить изображения карточек только при приближении к viewport; не трогать LCP-элементы; не ломать CLS; не создавать «пустоту» при быстром скролле бесконечной ленты.

### Гипотеза

`loading="lazy"` (нативный) для изображений + IntersectionObserver с `rootMargin` для сетки + буферизация следующей партии бесконечной ленты.

### Что получилось

```typescript
// Сетка каталога: корневой маржа 200px — загрузка до появления
...
- Первый экран каталога: выше fold — `loading="eager"` + `fetchpriority="high"` (только LCP-карточка, модуль 28), остальные первого экрана — eager без priority;
- сетка: IO с rootMargin 200px — изображения загружаются до того, как пользователь докрутил;
- бесконечная лента: следующая партия подгружается при буфере 3 карточек до конца — нет «пустоты» (антипаттерн №5).

### Грабли, найденные в production

1. **`loading="lazy"` на карточках первого экрана** — LCP-карточка каталога отложена (антипаттерн №1). Класс `.hero-card` исключён из lazy.
2. **Изображения без размеров** — карточки «прыгали» при загрузке: CLS на каждой второй карточке. `width`/`height` из БД (атрибуты приходят из 1С-прайса вместе с картинкой).
...
4. **Медленный каталог без буфера** — «докрутил до конца, лента думает»: буфер 3 карточки сделал подгрузку незаметной.

### Вывод, противоречащий интуиции

Lazy loading — не «добавить атрибут», а **три независимых решения**: что НЕ ленить (первый экран, LCP), как рано грузить (rootMargin), и как не показать пустоту (буфер). На каталоге выигрыш — в разы меньше запросов, но главное: CLS-регрессии и лаг скролла ловились именно настройками, а не атрибутом.

**Практический вывод для архитектора:** `loading="lazy"` + IO — минимальная пара для сеток; production-настройки: rootMargin, unobserve после загрузки, явные размеры, буфер подгрузки, исключения для above-fold. Ленить изображения без размеров — менять CLS-проблему на CLS-проблему.
