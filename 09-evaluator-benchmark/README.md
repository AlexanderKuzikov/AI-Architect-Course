...
### Дополнение из практики: слепые гейты стенда ревью (сентябрь 2026)

Полигон заморожен тегом, правится только стенд. Каждая модель видит только свой бриф и свой срез, чужие прогоны и эталоны запрещены, нарушение — прогон в брак. Шаги идут строго последовательно, гейт закрывает предыдущий.

Что сработало и стоит скопировать:

- Единое задание всем: один и тот же бриф на идентичном срезе, побайтовое сравнение файлов прикладывается к отчёту. Иначе сравниваешь не модели, а формулировки.
- Перекрёстная сверка: каждый разбор судят все остальные, эталон — то что подтвердили минимум четверо из пяти, спорное — на арбитраж штаба. В первом круге так закрыли пятьдесят семь пунктов полностью, часть понизили, три оставили спорными.
- Якоря в брифе: нулевой бриф дал разброс и завышенные severities, бриф с якорями и зонами дал прирост объёма у всех и срезал перегибы в разы. Вывод для архитектора: качество eval начинается с постановки, а не с судьи.
- Предполётный порядок: перед запуском показывается вся таблица модель — задание — срез — слот выхода и получается явное «гони» именно на неё. Строка расходов получает статус до траты токенов. Появилось после двух самовольных запусков.
- Судья проверяется так же строго: отдельная модель судила чужие списки тем же порядком, согласие вышло под восемьдесят процентов, строгость подтвердилась направлением — себя судит так же жёстко.

**Практический вывод для архитектора:** слепота + гейты + арбитраж дешевле, чем кажется, и ловят то что не ловит одиночный судья: самокоррекцию severities, уникальные находки (в одном круге только одна модель нашла TOCTOU проверка-выпуск без транзакции — пропустили десять прогонов), и эффект постановки.
