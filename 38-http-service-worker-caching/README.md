...
## 7. Реальный кейс: immutable-кеш и инвалидация на PHP-сайте

### Контекст

Корпоративный сайт услуг (PHP-шаблоны, модули 30–34): статика (CSS/JS/изображения) отдаётся веб-сервером, HTML — динамический PHP. Классическая схема: ассеты `immutable`, HTML `no-cache`.

### Задача

Раз в неделю — жалоба «не вижу новую версию страницы»: то стиль старый, то изображение не обновилось. Инвалидация — вручную, «потому что так делаем».

### Гипотеза

Проблема не в заголовках (они правильные: ассеты immutable + HTML no-cache), а в **процессе деплоя**: нет единого скрипта, имена ассетов не хэшируются, «горячие правки» файлов ломают контракт.

### Что получилось

```nginx
# nginx: статика с хэшами — immutable; HTML — revalidate
location /assets/ {
    # имена вида style.a1b2c3.css — генерируются при сборке
    add_header Cache-Control "public, max-age=31536000, immutable";
}

location / {
    # HTML всегда ревалидируется
...
```bash
#!/bin/bash
# deploy.sh — сборка с хэшами + деплой одной командой
npm run build   # генерирует style.<hash>.css, main.<hash>.js
rsync -a --delete dist/ server:/var/www/site/
```

- Ассеты: уникальные имена с хэшем — новый деплой = новые URL, старые файлы в кеше браузеров валидны для старых страниц;
- HTML: `no-cache` — браузер всегда спрашивает ревалидацию, PHP отдаёт свежую страницу;
- «ручная инвалидация» исчезла: при immutable-ассетах инвалидировать нечего — контракт имён.

### Грабли, найденные в production

1. **Пользовательский кеш не сдаётся**: даже с `no-cache` у пользователя в браузере мог остаться старый HTML (обходные кеши, превью-прокси). `stale-while-revalidate` для HTML вместо жёсткого no-cache: фоновая ревалидация.
2. **«Горячая правка» без пересборки**: поправили одну строчку в `assets/style.css` на сервере — тот же URL, старое содержимое в кешах на год. Правило: правки только через сборку с хэшами, никогда руками в dist.
3. **Пропуск `--delete` в rsync**: старые файлы копились годами — «ассеты с хэшами» превращались в свалку. `--delete` обязателен, иначе каталог растёт с каждым деплоем.

### Вывод, противоречащий интуиции

Кэширование — это **контракт имён**: immutable-ассеты работают, пока у каждого артефакта уникальное имя (хэш). Инвалидация нужна только HTML; «почистить кеш руками» — признак того, что контракт нарушен (меняем содержимое по старым URL). Половина проблем ушла, когда деплой стал скриптом со сборкой, а не «залили файлы вручную».

**Практический вывод для архитектора:** проектируй кэш как контракт: хэшированные имена для ассетов (immutable), revalidate для HTML, деплой = сборка + синхронизация с `--delete`. Любая «горячая правка» артефакта — нарушение контракта, ведущее к таинственным «старым версиям».
