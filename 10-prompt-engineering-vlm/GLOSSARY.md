...
Используется в grounding запросах к Qwen3.5.
...
**Conv3d (DeepStack)**
Трёхмерная свёртка в vision encoder Qwen3.5 для нативной обработки видео.
...
**DeepStack ViT**
Иерархический Vision Transformer в Qwen3.5.
...
В Qwen3.5 early fusion устраняет «шов» между визуальным и текстовым пониманием.
...
**Gated DeltaNet**
Линейный attention механизм в Qwen3.5 (часть Hybrid Attention).
...
**Hybrid Attention**
Архитектура attention в Qwen3.5: комбинация Gated DeltaNet (линейный) и Gated Attention (стандартный).
...
**Kimi K2.5**
Open-weight VLM от Moonshot AI (~1T MoE, Arena ELO ~1438, февраль 2026).
Vision поддерживается только через vLLM — GGUF и llama.cpp дают только text inference.
MoonViT-3D encoder, нативный мультимодал обученный с нуля.

## L

**Late Fusion**
Архитектурный подход предшественников Qwen3.5: vision encoder и LLM обучались раздельно,
...
**MoonViT-3D**
Vision encoder Kimi K2.5.
Нативная обработка видео через 3D свёртки.
Обучен совместно с LLM backbone с нуля — early fusion архитектура.

## P

**Patch**
Фрагмент изображения фиксированного размера (в Qwen3.5: 32×32 пикселя).
...
В Qwen3.5: 32×32. Определяет соотношение разрешение / количество visual tokens.
Итоговые размеры изображения должны быть кратны patch_size.

## R

**Reasoning Mode (VLM)**
Управление chain-of-thought в Qwen3.5 для визуальных задач.
...
Паттерн: маленькая модель (0.8B/2B) классифицирует сложность задачи,
результат направляется к модели нужного размера.
На GTX 1660: снижает median latency в 2–3× при неизменном качестве на сложных кейсах.
