...
Персональные данные шифруются на уровне приложения отдельным ключом, отличным от секрета токенов, рядом лежит наводка для поиска и дедупликации. Ключ шифрования — только из окружения с fail-fast в проде, в логи чувствительное не печатается никогда, шифротекст из ответов API снесён, чтение чувствительных полей — в журнальный след для закона о персональных данных.

Проверка бандла — строковым поиском без паролей и секретов: экран входа без захардкоженных паролей, токены не в локальном хранилище без необходимости, сид-пароли только из окружения с dev-фолбэками, которых нет на проде. Ревью ловит это тестами: тест, фиксирующий утечку в логах или бандле, — такой же gate, как тест логики.
