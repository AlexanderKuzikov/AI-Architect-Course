...
## 7. Реальный кейс: a11y-аудит сайта услуг — автоматы нашли треть

### Контекст

Корпоративный сайт услуг (модули 30/31): страницы услуг, формы заявок, модальное окно обратного звонка. Аудит перед релизом: axe + Lighthouse accessibility score — «ошибок нет».

### Задача

Валидный a11y-аудит без ручной проверки — ложная уверенность: автоматические проверки находят < 30% проблем (WCAG-известный факт, см. Anti-checklist). Нужны ручные сценарии для критичных flow: форма заявки, модалка, навигация.

### Гипотеза

Три ручных сценария закрывают 80% реальных проблем: заполнение формы заявки с клавиатуры, модалка обратного звонка, навигация по меню со screen reader.

### Что получилось — проблемы, которые не нашёл ни один автомат

1. **Фокус-ловушка в модалке**: Tab в модалке обратного звонка уводил фокус на фон (страница скроллилась под модалкой). Фикс — focus trap: цикл Tab внутри модалки, фокус на открытии, возврат на кнопку при закрытии.
2. **Форма заявки: `role="checkbox"` на `<div>` без клавиатурного обработчика** — фокус есть, Space/Enter не работают (антипаттерн №6 модуля). Фикс — нативные `<input type="checkbox">` вместо дивов.
3. **Ошибки валидации без `aria-live`**: «не заполнено поле» меняло текст под инпутом, но screen reader молчал — «нажал отправить, ничего не произошло». Фикс — `aria-live="polite"` на контейнере ошибок формы + `aria-describedby` на инпутах.
4. **Контраст текста в подвале**: серый текст на белом — 3.1:1 при норме 4.5:1. Автоматы могут ловить, но цвет «рабочий» проскочил — ручная проверка на реальном контенте.

### Вывод, противоречащий интуиции

Самые дорогие a11y-баги — не «отсутствие aria-атрибутов», а **сломанная интерактивность**: фокус, клавиатура, уведомление об изменении. Это архитектурные проблемы компонентов (как построена модалка, как устроена форма), а не «добавить атрибут». Автоматы видят атрибуты, но не поведение.
