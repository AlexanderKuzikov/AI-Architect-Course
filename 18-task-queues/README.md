...
Если потребителя ещё нет — ретранслятор не пишется до контракта спутника: журнал честно копится как durable-накопитель, флаг публикации пуст значит «не опубликовано», а не баг. Консьюмер, мёртвые письма и политики — за спутником при появлении контракта, ключ идемпотентности уже готов к ретраям без дублей. Объём растущей таблицы — мониторить, ретеншн следующим решением. Уведомления тем же транспортом — порт плюс адаптеры каналов, отложенные задачи; канал, требующий обхода блокировок, исключён для важных сообщений — активный канал это конфиг, не код.
