...
## 7. Реальный кейс: WASM-память, которую GC не видит

### Контекст

PDFtoText: рендер PDF через PDFium WASM в worker pool (модули 16/22). Сервис обрабатывает 500 PDF/день. Через две недели непрерывной работы — RSS процесса 1.8 GB при рабочем профиле ~300 MB, воркеры падают по OOM.

### Задача

Найти утечку в long-running сервисе: JS heap в норме (heapUsed 150 MB), воркеры перезапускаются, но память процесса растёт. Стандартные инструменты (Node --inspect, heap snapshot) показывают «всё чисто».

### Гипотеза

Утечка не в JS heap: WASM linear memory — отдельная область, которую GC не освобождает. PDFiumDocument в JS heap — маленькая обёртка, реальная память документа — в WASM. Без явного `destroy()` документ остаётся в памяти навсегда.

### Что получилось

```typescript
// ❌ Было: GC «освободит сам» — не освобождает WASM-область
const doc = library.loadDocument(pdfBuffer)
const text = doc.getPage(0).getText()
// doc нигде не уничтожается

// ✅ Стало: destroy() в finally — обязателен на каждом пути
const doc = library.loadDocument(pdfBuffer)
try {
  for (let i = 0; i < pageCount; i++) {
    allTexts.push(doc.getPage(i).getText())
  }
} finally {
  doc.destroy()   // ✅ освобождает WASM heap независимо от ошибок
}
```

- Диагностика: профиль WASM heap (`--experimental-wasm-memory-profiling`) показал рост удерживаемой памяти документов; JS heap-снапшоты были чистыми;
- утечка: каждый обработанный PDF оставлял в WASM-памяти ~10–50 MB (страницы, bitmap) — 500 PDF/день × неделя = гигабайт;
- фикс: `destroy()` в `finally` + проверка в контрактных тестах (модуль 21: «document.destroy() после каждого документа» — golden-проверка).

### Грабли, найденные в production

1. **JS heap-профилирование не видит WASM**: `--inspect` + heap snapshot — «утечки нет», а процесс умирает. Для WASM-библиотек мониторить WASM heap отдельно (модуль 16: чеклист «WASM heap мониторится отдельно от JS heap»).
2. **Утечка в обёртке, не в библиотеке**: PDFium сам чистый; утечка — наш код, забывший destroy. Тесты на lifecycle (создал → уничтожил) — часть контракта.
3. **Worker pool маскировал утечку**: воркер с OOM перезапускался, «сбрасывая» память — сервис работал месяцами с периодическими рестартами, пока не выросла частота падений.

### Вывод, противоречащий интуиции

Самая дорогая утечка не видна в JS-профилере: память native/WASM — вне GC, и «JS GC сам освободит» работает только для JS. Long-running сервисы с WASM/native-библиотеками обязаны иметь явный lifecycle (destroy/free) и отдельный мониторинг памяти — иначе утечка живёт месяцами, маскируясь перезапусками.

**Практический вывод для архитектора:** любая интеграция с WASM/native-модулями — контракт lifecycle: «кто создал, тот и уничтожил», finally-гарантия, тест на утечку в CI. Для Node-сервисов с WASM — отдельная метрика WASM heap в мониторинге (модуль 26).
...
Формула: heap snapshot (3 замера) + дифф по классам/строкам + список проблем с фиксами на ревью.
