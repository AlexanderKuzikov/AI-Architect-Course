...
### Что делать при расхождении Lab/Field — по шагам

```
1. Убедиться, что расхождение реальное: Lighthouse на staging ≠ prod
   (prod может иметь CDN-кеш, которых нет на staging)

2. Открыть CrUX по URL, посмотреть p75 по formFactor:
   — «Field плохой только на PHONE» → реальность мобильных, lab не виновата
   — «Field плохой везде» → искать systemic причину (third-party, шрифты)

3. INP невозможно измерить в lab → искать по LoAF (модуль 33) у реальных
   пользователей: Long Animation Frames = источники блокировок main thread

4. Сравнить с web-vitals RUM: собственные данные детальнее CrUX
   (CrUX — p75 за 28 дней, RUM — любой перцентиль за любое окно)

5. Фикс → деплой → ждать 28 дней для подтверждения в CrUX
   (RUM покажет эффект за дни, CrUX — за месяц)
```

---

## 7. Реальный кейс: CWV в SEO-проекте — поле решает, lab не спорит

### Контекст

SEO-мониторинг (SerpWatcher — отслеживание позиций по запросам): после редизайна сайта позиции по коммерческим запросам просели в течение 3 недель. Владелец связал падение с редизайном, но Lighthouse на staging показывал 90+ по performance.

### Задача

Понять, что изменилось для поисковой системы, и вернуть позиции. Кандидат №1 — Page Experience сигнал: Core Web Vitals в field data.

### Гипотеза

Lighthouse 90+ ≠ хороший field: реальные пользователи на мобильных, с кешем и third-party, видят другое. Проверяем CrUX и собственный RUM до начала «оптимизаций вслепую».

### Что получилось

1. **CrUX по ключевым URL**: LCP и INP на PHONE — Poor, хотя DESKTOP — Good. Редизайн добавил тяжёлый hero-слайдер (LCP-изображения менялись в процессе загрузки — модуль 28 §2) и third-party виджет (INP).
2. **RUM через web-vitals + sendBeacon** показал детализацию, которой нет в CrUX: худшие взаимодействия — клики по карточкам товаров (тяжёлый обработчик фильтра).
3. **Lab не соврал, он просто не тот инструмент**: Lighthouse не имеет реальных кликов (INP неизмерим) и не воспроизводит мобильные устройства пользователей.

### Грабли, найденные в production

1. **Ставить INP-фиксы по Lighthouse** — бессмысленно: lab-инструмент не даёт INP. Правду даёт LoAF у реальных пользователей (модуль 33) и RUM.
2. **Оптимизировать только главную** — CrUX по origin: худший URL тянет весь origin в Poor. Проверялись страницы каталога, которые генерируют 80% трафика, а не homepage.
3. **28-дневное окно CrUX** — фиксы не «подтверждались» неделями; собственный RUM (дни) стал первичным источником, CrUX — подтверждением для ранжирования.

### Вывод, противоречащий интуиции

Улучшение Lighthouse 90→95 не дало ничего для позиций: они вернулись после фиксов, найденных через **field data** (мобильные LCP + INP), которые lab-инструмент не показывал вовсе. В SEO-проекте CWV — это не «технический долг сайта», а метрика ранжирования: мерить нужно ровно то, что видит поисковик, — field data.

**Практический вывод для архитектора:** для сайта, где CWV влияет на бизнес (SEO-трафик), бюджет на перф — это RUM + CrUX + фиксы по field data, а не «поднять Lighthouse до 95». Lab — для CI-гейтов (регрессии), field — для реальности. Одно без другого даёт ложную уверенность или ложную тревогу.
...
Формула: LCP-элемент + fetchpriority/preload + width/height + AVIF/WebP через sharp + ограничение правок (только above-fold).
...
Формула: img без размеров → width/height или aspect-ratio + font-display (optional для контентных) + min-height для динамики.
