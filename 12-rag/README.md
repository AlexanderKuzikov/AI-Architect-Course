...
Компоненты [^1]–[^3] образуют жёсткую цепочку зависимостей: изменение chunking или embedding модели инвалидирует весь индекс и требует переиндексации. Это не просто «медленно» — это операция, которую нужно планировать как migration.
...
function assembleContext(
  ...
  const reordered = sorted.length > 2
    ? [sorted, ...sorted.slice(2), sorted]  // top-1 первый, top-2 последний[^1]
    : sorted;
