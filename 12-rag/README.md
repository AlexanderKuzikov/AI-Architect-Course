...
## 9. Реальный кейс: RAG по судебным документам

### Контекст

Судебный документооборот (CourtDesk): мониторинг дел — карточки с сайтов судов (стороны, события, решения), PDF-документы (~500/день: решения, договоры, сканы 1С), события по делам. Потребитель — 1С-интеграция и юридический UI: вопросы «по каким делам у контрагента были решения за последний квартал», «что решил суд по делу X».

### Задача

Поиск по документам двух классов:

| Класс запроса | Пример | Инструмент |
|:--|:--|:--|
| Точный (структурированный) | номер дела, УИД, ФИО стороны, дата | SQL-индекс, полнотекстовый (модуль 49) |
| Семантический (неструктурированный) | «дела, где суд взыскал с поставщика неустойку по договору» | RAG |

### Гипотеза

RAG — только для семантического класса. Точный поиск по номеру дела через вектора — антипаттерн: эмбеддинг номера «1-123/2026» не сравнивается надёжно (модуль 49: нормализация номеров — TEXT-канон до хранения). Гибрид: точный поиск — по индексу, семантический — по векторному слою.

### Архитектура (решения, а не вымысел)

```text
Сбор (парсеры судов, капча, WAF-backoff — CourtDesk)
   → нормализация (номер дела, УИД, кодировки CP1251→UTF-8)
   → извлечение текста: PDFium.getText() / render → VLM OCR (модули 15/16)
   → чанкинг по структуре карточки (не по токенам):
       карточка = события + стороны + решение — чанки по блокам
   → embedding + model_version (модуль 49: дрейф эмбеддингов)
   → гибридный retrieval:
       точный (номер/ФИО/даты) → PostgreSQL
       семантический → pgvector top-k + rerank
   → context assembly + citations (обязательно для юриспруденции)
```

Ключевые решения:

1. **Чанкинг по структуре, а не по токенам**: судебная карточка — блоки (стороны, события, решение, даты). Чанк = блок, а не 1024 токена нарезки вслепую — «вектор по блоку» отвечает на вопрос «в каких делах/блоках это было», а не «в каком куске текста».
2. **model_version на каждый эмбеддинг** (модуль 49): смена embedding-модели делает старые векторы несовместимыми — фильтр по версии обязателен.
3. **Инкрементальная индексация по событиям**: дела живут (события добавляются, решения вступают в силу) — переиндексация не «всего», а изменённых блоков по `updated_at` (модуль 18: инкрементальный сбор).
4. **Temporal awareness**: `legalForceDate`, версии документов — вопрос «по состоянию на дату» обязан учитывать актуальную версию (§8 Temporal RAG).
5. **Verification обязателен**: юридический ответ без ссылки на документ — не ответ. Citations per claim + abstain policy (нет уверенности — сказать «не найдено», не галлюцинировать).

### Грабли (реальные, из практики домена)

1. **Текст из 1С-сканов не извлекается** — битый ToUnicode, PUA-символы (модули 15/16): в векторный слой шёл мусор. Маршрут: PUA-диагностика → render → VLM OCR (модуль 10), только потом чанкинг.
2. **Номер дела в эмбеддинге** — «1-123/2026» и «1-123-26/2026» дают разные векторы (неразрывные дефисы!). Нормализация до хранения (модуль 49) — предусловие индексации.
3. **Кодировки**: ГАС/1С-источники отдают CP1251 — текст попадал в корпус с «кракозябрами», recall падал молча. Детект кодировки на входе (knowledge cp1251-gas).
4. **Дубли при повторном сборе**: карточка пере-парсится при мониторинге — переиндексация блока вместо «вставить ещё раз» (идемпотентность, модуль 51: бизнес-ключ).
5. **События добавляются чаще, чем меняется решение**: «последняя версия» блока — по updated_at, не «первый попавшийся».

### Метрики

> ⚠️ Плейсхолдеры — реальные значения будут вставлены автором (не выдумывать).

- Recall@k на семантических вопросах: `[МЕТРИКА: заполнить]`
- Точность извлечения дат/сумм из решений (schema-валидация): `[МЕТРИКА: заполнить]`
- Cost/день на эмбеддинги + VLM OCR: `[МЕТРИКА: заполнить]`
- Latency p95 запроса (retrieval → ответ): `[МЕТРИКА: заполнить]`
- Доля ответов с abstain («не найдено» вместо галлюцинации): `[МЕТРИКА: заполнить]`

### Вывод, противоречащий интуиции (структурно)

Главный вывод из проектирования: **в судебном домене точный поиск ценнее семантического** — большая часть реальных запросов 1С/юристов структурна (номер, ФИО, даты), и RAG для них — потеря надёжности. RAG оправдан на текстах решений/договоров, где нет структуры. Гибрид «индекс для точного + вектора для семантики» — не компромисс, а правильная архитектура домена.

Второй вывод: RAG в юриспруденции невозможен без verification и temporal awareness — ответ без ссылки и с устаревшей версией документа хуже отсутствия ответа.

**Практический вывод для архитектора:** прежде чем строить RAG — классифицируй запросы домена (точные vs семантические). Суды/документы/финансы — гибрид по умолчанию; чистый RAG оправдан только там, где запросы неструктурированы и ошибка дешёвая.
