...
Запрос данных:
  ...
  ┌──────────────────────────────────────────────────────────┐
  │  L1: In-process memory (LRUCache / Map)                  │
  │  Latency: 0.001–0.1ms  |  Size: 10–500 MB RAM           │
  │  Scope: один процесс   |  Eviction: LRU / TTL           │
  └──────────────────────────────────┬───────────────────────┘
                           MISS      │
                                     ▼
  ┌──────────────────────────────────────────────────────────┐
  │  L2: Redis (distributed cache)                           │
  │  Latency: 0.5–5ms      |  Size: гигабайты               │
  │  Scope: все инстанции  |  Eviction: allkeys-lru / TTL   │
  └──────────────────────────────────┬───────────────────────┘
                           MISS      │
                                     ▼
  ┌──────────────────────────────────────────────────────────┐
  │  L3: Source of truth (PostgreSQL / API / S3)             │
  │  Latency: 5–500ms      |  Size: неограниченно            │
  │  Scope: все            |  Eviction: нет                  │
  └──────────────────────────────────────────────────────────┘

CDN: между клиентом и сервером
  Latency: 5–50ms (edge)  |  Scope: публичный контент
  Eviction: Cache-Control заголовки
```
...
> Кандидаты: кэш embeddings снизил стоимость AI API, L1+L2 стратегия для справочников WooCommerce, CDN инвалидация при обновлении каталога.
