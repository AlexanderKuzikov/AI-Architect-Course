...
**API-контракт** ([Модуль 51](../51-api-design/README.md))
Стабильный интерфейс между независимыми системами: ресурсы, коды ошибок, форматы, версия. Поля только добавляются; семантические изменения требуют новой версии. Contract-first: OpenAPI-спецификация — единственный source of truth, контрактные тесты в CI. Для 1С-потребителя — плоский JSON, ISO 8601, идемпотентные POST.

**API key** ([Модуль 51](../51-api-design/README.md))
Длинная случайная строка (≥ 32 байта) для аутентификации сервис-потребителей (1С, cron). Передаётся в `Authorization: Bearer <key>`. Для сервис-интеграций предпочтительнее JWT: нет refresh-цикла; ротация парой активных ключей; ключи не логируются.

## B

**Bulkhead** ([Модуль 52](../52-resilience-patterns/README.md))
Изоляция ресурсов между потребителями: отдельные пулы слотов (семафоры) на сегмент — перегрузка одного сегмента (парсер, 1С-выгрузка) не истощает слоты другого (UI). Слот занят на время выполнения; ожидание слота — с таймаутом. Дополняет rate limit, не заменяет его.
...
**Circuit Breaker** ([Модуль 52](../52-resilience-patterns/README.md))
Паттерн с тремя состояниями: CLOSED (запросы идут, ошибки считаются), OPEN (fail fast), HALF_OPEN (пробные запросы). Защищает downstream от каскадов: после порога ошибок вызовы мгновенно отклоняются. 429 не считается ошибкой CB (это сигнал «сбавь темп», не «сервис упал»); retry — снаружи CB, не внутри.

**Cursor-пагинация** ([Модуль 51](../51-api-design/README.md))
Пагинация по позиции в строгом порядке сортировки `(updated_at, id)`. Курсор — непрозрачная для клиента строка (`next_cursor`); конец выгрузки — `next_cursor: null`. Для выгрузок (1С-регламентные задания) вместо offset: не даёт пропусков/дублей при изменении данных.
...
**Idempotency-Key** ([Модуль 51](../51-api-design/README.md))
Заголовок, который клиент шлёт с POST-операцией; сервер хранит результат по ключу (TTL 24–48 ч) и при повторном запросе возвращает сохранённый ответ. Применяется для произвольных операций; для импорта сущностей — идемпотентность по бизнес-ключу (ON CONFLICT).
...
**Retry** ([Модуль 52](../52-resilience-patterns/README.md))
Повтор запроса при сбое: только 408/429/5xx и сетевые обрывы; 4xx не ретраятся. Политика: exponential backoff + full jitter, `Retry-After` первичен, бюджет (maxAttempts/maxDelay), идемпотентность операции, один ретрай-слой на пути. Retry storm: синхронные повторы множества клиентов создают пики на падающий сервис — до блокировки IP.
...
**Saga** ([Модуль 52](../52-resilience-patterns/README.md))
Распределённая операция из локальных шагов с компенсациями: при отказе шага N откатываются шаги 1..N−1. Замена ACID-транзакции между сервисами. Правила: идемпотентные шаги (модуль 51), состояние саги в БД с возобновлением после рестарта, компенсации с повторами и DLQ. Orchestration — для явных последовательностей, choreography — для событийных.
...
**Timeout (таймаут)** ([Модуль 52](../52-resilience-patterns/README.md))
Жёсткий лимит ожидания: на соединение, на вызов, общий бюджет запроса. Фундамент resilience: без таймаутов retry и bulkhead работают с «мёртвыми» соединениями, слоты копятся. Deadline распространяется через context/AbortSignal; общий таймаут сервера ≤ таймаута потребителя.
...
**Пагинация курсорная** ([Модуль 51](../51-api-design/README.md))
См. Cursor-пагинация: выгрузки больших объёмов по строгому ключу `(updated_at, id)` вместо offset. Потолок `limit`, непрозрачные курсоры, `next_cursor: null` — явный конец.
...
**Гейт (quality gate)** ([Модуль 09](../09-evaluator-benchmark/README.md))
Порог закрытия шага стенда: шаг принят только целиком, следующий начинается после закрытия предыдущего. В слепом стенде гейт закрывают все модели, а не одна.

## Д

**Диспетчер задач** ([Модуль 42](../42-a2a-protocol/README.md))
Вечный файл-указатель штаба: постоянная часть (как работать) плюс сменная (текущее задание, статус, счётчик, архив). Правят только штаб, сессии его не трогают.

## З

**Замороженный полигон** ([Модуль 09](../09-evaluator-benchmark/README.md))
Код для тестов зафиксированный тегом и читаемый только на чтение. Эксперименты идут в стенде, полигон не правят никогда.

**Заморозка объёма** ([Модуль 42](../42-a2a-protocol/README.md))
Слово штаба посреди сессии объём не меняет. Новая работа — правка файла задачи с новой приёмкой, отдельный номер в очередь или откладывание.

## С

**Слепой прогон** ([Модуль 09](../09-evaluator-benchmark/README.md))
Сессия видит только свой бриф и свой срез. Чужие прогоны, оценки и эталоны запрещены, нарушение — прогон в брак.

**Связка моделей** ([Модуль 11](../11-multi-model-orchestration/README.md))
Пара или тройка разных моделей для поиска багов веером: покрытие тем парой и тройкой выше одиночки, уникальные находки ловятся только веером. Пересчитывается при выбывании модели.

**Суггестивный контур (suggest-approve)** ([Модуль 06](../06-prompt-engineering/README.md))
Модель предлагает нормализованный текст и пару suggestion-полей рядом, решение остаётся за человеком с ручной правкой после автозаполнения.

## Ш

**Штаб-сессия** ([Модуль 42](../42-a2a-protocol/README.md))
Основная сессия команды: обсуждения, планирование, задания, приёмка. Кодовые сессии получают задание файлом, выполняют, пишут отчёт в тот же файл.

## Э

**Эталон сверки** ([Модуль 09](../09-evaluator-benchmark/README.md))
Находка подтверждённая минимум четырьмя из пяти судей при перекрёстной сверке. Спорное уходит на арбитраж штаба, а не в мусор.
