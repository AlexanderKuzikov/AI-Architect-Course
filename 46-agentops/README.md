...
Стенд гонял семь моделей по одному заданию и писал дельту дня в журнал: одна модель — шесть центов, две — по три цента, одна — четыре цента, одна — ноль по акции с оговоркой про смешанное потребление. Вывод не в цифрах, а в процессе: цена фиксируется до запуска, а не после счёта.

Три правила, которые стоит забрать в AgentOps:

- Статус трат до запуска: строка расходов получает пометку рейтинг-инструмент-фон до траты токенов. Команда запуска всегда называет задание каждой модели явно.
- Таблица запусков перед стартом: модель — задание — срез — слот выхода показывается целиком, запуск только после явного подтверждения именно этой таблицы. Лечит самовольные прогоны и несравнимые срезы.
- Cost attribution как в fallback: запрос ушедший на другую модель помечается фактической моделью, иначе дашборд врёт. В стенде так же врала бы матрица без указания брифа: один и тот же разбор на нулевом и якорном брифе — разные результаты, сравнивать их без пометки брифа нельзя.
