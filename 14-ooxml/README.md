...
### Дополнение из практики: SaaS-генерация как второй адаптер (август 2026)

Когда шаблон держат не разработчики, а юристы в SaaS-конструкторе, генерация уходит наружу: логин с парой токенов, генерация по идентификатору шаблона с форматом docx или pdf, ответ с base64 файла и медиа-типом. Формат шаблона — архив с описанием полей, самим docx с маркерами выбора и циклов и метаданными; функции дат, валют и склонений исполняются на стороне сервиса, в данных шлют сырые значения.

Ключи полей плоские из описания шаблона, группы не дают вложенности, списки — массивом объектов, даты — строго ISO. Ошибки по делу: нет аккаунта, неверные данные, шаблон не найден, невалидные данные. Офлайн-замена тем же контрактом — локальный шаблонизатор, чтобы смена адаптера не меняла потребителей. Правило: контракт генерации фиксируется один раз, адаптеров может быть два.
