...
### Провайдеры AI API и российский рынок

Выбор провайдера влияет на HTTP-клиент меньше, чем кажется: все major AI API — OpenAI-compatible (`POST /chat/completions`, Bearer-ключ). Разница — в граблях:

| Провайдер | Особенности | Грабли для HTTP-клиента |
|:--|:--|:--|
| OpenAI | стандарт | SSE-стриминг, Retry-After |
| Groq | free tier, жёсткие лимиты | 429 + Retry-After, см. кейс §11 |
| RouterAI (РФ) | рубли, маршрутизация по цене | `error` — строка, не объект; PDF direct; `</think>`-обрывки у VLM |
| OpenRouter | ротация ключей, fallback | умная маршрутизация между вендорами |

Практика RouterAI (подробно в knowledge/routerai-api.md):
- парсить `error` в обоих вариантах: строка или `{message}`;
- сломанный PDF → HTTP 200 с `<file status="error">` маркером в контенте — детектить, а не доверять ответу;
- `reasoning.effort: none` отключает думание (reasoning_tokens=0);
- поведение параметров может различаться между провайдерами из-за маршрутизации.

Для российского продакшена: тот же undici/got-клиент, но с валидацией ответа под конкретного провайдера (refusal-детектор, маркеры ошибок, strip `</think>`).
...
### Дополнение из практики: формы ошибок AI-провайдеров (август 2026)

Два провайдерских грабля, которые ломают наивную нормализацию выше:

- Ошибка строкой: поле ошибки в ответе бывает строкой, а не объектом с сообщением. Парсить оба варианта, иначе «ошибка без сообщения» уйдёт в network.
- Двести с маркером ошибки внутри: сломанный PDF роутер отвечает кодом двести, а парсер вставляет в контент модели маркер с именем файла и текстом. Модель «отвечает» на маркер. Детектить маркер регуляркой до JSON-парсинга и поднимать как ошибку парсера, с ретраем бессмысленно — документ надо чинить.

Оба случая — не retry по статусу, а ветка «чинить вход или ключ»: дешёвый ping-запрос проверяет ключ до батча, маркер — входной файл до повтора.
...
**Задача:** надёжный HTTP клиент для Groq API (free tier).
...
Стандартный retry **усугубил** проблему rate limit.
Каждый retry после 429 тратил один из 30 RPM:

```
Попытка 1: 429 → ждём 1s | Попытка 2: 429 → ждём 2s
Попытка 3: 429 → ждём 4s | Попытка 4: job → DLQ
```

Три retry × 30 запросов = 90 RPM при лимите 30. Rate limit
срабатывал на каждой второй попытке. Решение:
`retries: 2` (не 3) + читать `Retry-After` headers +
уважать `x-ratelimit-remaining-requests`.

Вторая неожиданность — **connection pool exhaustion**.
undici Pool connections=10, Groq отвечает 2-10s. 10 соединений
заняты → 11-й запрос ждёт → TCP connect заново (80ms overhead).
Решение: pool=20 + keepAliveTimeout=60s + мониторинг пула.

Третья проблема — **circuit breaker открывался при rate limit**.
opossum считает 429 ошибкой. При 30 запросах/сек с rate limit —
CB открывается за 2 секунды. Решение: 429 обрабатывается до CB
(в retry логике), CB считает только 5xx и таймауты.

**Вывод, противоречащий интуиции:**

Самая частая причина падения pipeline — не «API не отвечает»,
а «retry сожрал весь RPM». Меньше retry (2 вместо 4) + уважение
Retry-After = стабильнее чем агрессивный retry.

| Стратегия | Success rate | RPM на retry | Среднее время |
|-----------|-------------|-------------|---------------|
| Retry 4, no Retry-After | 94% | 180 (6×) | 45s |
| Retry 2 + Retry-After | 98.5% | 12 (0.4×) | 18s |
| + pool=20 + мониторинг | 99.2% | 8 | 12s |
...
- [ ] Один глобальный таймаут на все запросы — health check и AI inference живут в разных мирах
- [ ] Retry без jitter — multi-instance деплой устроит retry storm при первом сбое
- [ ] Retry для 4xx (кроме 429) — 400 Bad Request не станет 200 с повторной попытки
- [ ] Circuit breaker без fallback — OPEN = `CircuitBreakerOpenError`, пользователь без ответа
- [ ] Новый `got.extend()` instance на каждый запрос — TCP + TLS per request = 80–350ms overhead
- [ ] Retry внутри circuit breaker — CB не видит ошибок, никогда не откроется
- [ ] `npm install axios` без пина после инцидента 31.03.2026 — supply chain риск
