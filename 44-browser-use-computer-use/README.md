...
### Дополнение из практики: мост через залогиненный браузер и shared-хостинг (сентябрь 2026)

Два свежих подтверждения тезиса выше.

Мост к кабинетам поисковика: API умеет диагностику, sitemap и переобход, но не умеет добавить хост и управлять счётчиком — это только кликами в залогиненном профиле. Правила из жизни: работать текстовыми снапшотами без скриншотов — скриншоты копятся в истории и убивают сессию лимитом; клик по подписи согласия может открыть соглашение в новой вкладке вместо галочки — жать чекбокс напрямую и проверять флаг; номер счётчика забирать из адреса после создания; файл подтверждения класть в корень сайта и коммитить как артефакт деплоя; код счётчика проверять текстовым поиском по HTML.

Выживание на shared: FTP-логин проходит, а листинг на любом пути отдаёт один файл про грейлист — это не права, а сеть; снятие — капча владельца в браузере за полминуты, автоматом не проходится. Пароль из конфига файл-менеджера читать через внутренний текст узла с учётом кодировки, иначе будет мусор и отказ логина. Живые проверки дублировать вторым HTTP-стеком: один рвётся на renegotiation и пустых ответах, второй проходит — это не падение сайта. Дефолтный питоний юзер-агент режется защитой, проверки — с браузерным.
