...
## 6. Реальный кейс: квота как бюджет

### Контекст

Сервис с тремя разными потребителями внешних лимитов:

| Потребитель | Внешний лимит | Окно |
|:--|:--|:--|
| LLM-пайплайн (Groq free) | 30–60 RPM / 1 000–14 400 RPD по моделям | rolling 60s / UTC midnight |
| Сбор судебных карточек | IP-баны сайтов судов, капча на N-ю сессию | нет явного окна — «до бана» |
| SerpWatcher (позиции) | лимиты поисковых API, ~14-дневное окно данных | час/сутки |

### Задача

Лимиты внешних провайдеров — конечный ресурс, как бюджет. Критичные задачи (юридический мониторинг дел) не должны голодать из-за некритичных (batch-обработка архива), а batch не должен «съесть» суточную квоту в первые минуты.

### Гипотеза

Rate limiting — не «защита от спама», а **планирование бюджета**: раздельные лимитеры по задачам + резервирование квоты под критичные + учёт разных окон сброса (модуль 11).

### Что получилось

1. **Резерв квоты**: 30% суточной Groq RPD зарезервированы под критичные задачи (отдельный лимитер), 70% — общий пул для остальных. Критичная задача не падает в конце дня, когда пул исчерпан.

```typescript
// Два лимитера на один ключ — приоритетный пул и общий
const critical = new RateLimiterRedis({ points: 300, duration: 86400 });  // 30% от 1000 RPD
const general  = new RateLimiterRedis({ points: 700, duration: 86400 });  // 70%
```

2. **Учёт окон сброса**: RPM-окно (rolling 60s) и RPD-окно (UTC midnight) — разные счётчики. Лимитер по RPD без учёта RPM ловил 429 от самого Groq: 60 запросов за минуту при лимите 30 RPM = 429, хотя RPD ещё не исчерпан.

3. **SerpWatcher**: инкрементальный сбор (модуль 18) — «пропустить уже проверенные сегодня» — экономит лимит поискового API: повторные проверки не тратят квоту.

4. **Судебный сбор**: лимитер не по запросам, а по «стоимости»: капча (RuCaptcha) дороже запроса — лимит на сессии, WAF-backoff (модуль 44) при 403.

### Грабли, найденные в production

1. **Очередь перед лимитером, а не после**: запрос, получивший 429 на внешнем API, списал токен лимитера — квота потрачена, результат не получен. Правильно: rate limit на исходящие до вызова, 429 внешнего API — отдельный сигнал (Retry-After), а не списание своего лимита.
2. **Один лимитер на два окна** — смешивание RPM и RPD: лимитер по «запросам» не различал окна; понадобились два счётчика с разными duration.
3. **`blockDuration` без логирования**: ручной бан IP суда срабатывал тихо — блокировки стали логироваться с причиной (модуль 26: request_id + причина в логе).

### Вывод, противоречащий интуиции

Rate limiting на исходящие вызовы — не «тормоз для скорости», а **механизм распределения дефицита**: квота провайдера распределяется между задачами по приоритету, иначе её съедает первая запущенная задача. Раздельные лимитеры + резерв + учёт окон сброса — это бюджетная политика, а не middleware-строчка.

**Практический вывод для архитектора:** для внешних API с квотами считай лимиты бюджетом: раздельные пулы по приоритету, резерв под критичное, отдельные счётчики под разные окна (RPM/RPD), лимитер до вызова, а не после. Иначе «некритичный» batch роняет критичный мониторинг.

### Дополнение из практики: лимит входа (сентябрь 2026)

Та же механика внутрь: попытки входа — пять на четверть часа с блокировкой. Защита от перебора — не «капча на форму», а счётчик на сервере. Правило: лимит на запись и вход жёстче, чем на чтение; срабатывание лимита — в лог с причиной, молчаливый бан расследуется неделями.
...
- [ ] Один глобальный лимитер для всех эндпоинтов — /health и /ai/generate имеют разную стоимость
- [ ] Rate limiting только на application уровне — DDoS с 10K req/sec убьёт процесс до middleware
- [ ] 429 без Retry-After — клиент не знает когда повторить, делает retry storm
- [ ] IP-based rate limiting за NAT — 500 пользователей за одним IP получают один лимит
- [ ] blockDuration без логирования — ложные срабатывания незаметны
- [ ] Rate limiting без идемпотентности — ретрай дорогой операции списывает токен дважды
...
Формула: rate-limiter-flexible + Redis + лимитеры по группам (общий/AI/брут) + insurance fallback + заголовки 429 + keyPrefix/Cluster.
...
Формула: фабрика лимитеров по планам + синглтоны + выбор по req.user.plan + anonymous fallback per IP + trust proxy.
