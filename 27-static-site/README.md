...
### Языковые версии (i18n) — где ломается

Многоязычный SSG — классический источник дублей контента:

| Конфигурация | URL | Риск |
|:--|:--|:--|
| `/en/`, `/ru/` поддиректории | `site.com/en/blog/post` | дубль без hreflang |
| Поддомены `en.site.com` | зеркало, дороже | раздельный CDN-кеш |
| Один URL + `lang` cookie | ❌ | поисковики не индексируют cookie-контент |

```html
<!-- hreflang — обязательная пара для каждого перевода -->
<link rel="alternate" hreflang="en" href="https://site.com/en/blog/post" />
<link rel="alternate" hreflang="ru" href="https://site.com/ru/blog/post" />
<link rel="alternate" hreflang="x-default" href="https://site.com/en/blog/post" />
```

**Почему это важно архитектору:** i18n в SSG решается в build (двойная генерация), а не в runtime (cookie/редиректы). Каждая языковая версия — отдельный набор страниц в build time: объём, кэш и sitemap считаются до старта.

### RSS/feed для контентных сайтов

Feed-генерация в build — часть SSG-пайплайна: не забыть при добавлении новых типов контента. `changefreq`/`lastmod` в sitemap и `lastBuildDate` в RSS должны генерироваться из данных, а не быть статикой.

---

## 6. Реальный кейс: SSG без фреймворка — 52 модуля в один HTML

### Контекст

AI-Architect-Course: 52 модуля в Markdown (мультиязычные примеры кода), глоссарий, служебные страницы. Задача — читаемый единый документ: навигация по трекам, полнотекстовый поиск, темы, работа офлайн.

### Гипотеза

Фреймворк (Astro/Next) для закрытой документации без SEO и без персонализации — оверинжиниринг. Достаточно своего сборщика на Node: markdown-it + модули → один HTML.

### Что получилось

`build-html.mjs`: 52 модуля → секции одного HTML (course.html, ~6 MB), sidebar с треками (Languages → Resilience), клиентский поиск по модулям, глоссарий со ссылками между модулями (`glossary-links.mjs`), темы оформления.

```text
52 × README.md + GLOSSARY.md
   → markdown-it (build time)
   → один course.html (все модули + глоссарий + карта курса)
   → 0 KB серверного JS, поиск на клиенте
```

- Build ~2 секунды на 52 модуля — «масштаб» из §4 решается тривиально;
- Кросс-ссылки между модулями проверяются скриптом (битая ссылка = fail, модуль 25);
- Единственный файл — удобно читать офлайн, один артефакт для деплоя.

### Грабли, найденные в production

1. **6 MB одним файлом** — компромисс: один HTML тяжелее 52 страниц, но поиск и навигация покрывают потерю. Для SEO-сайта так нельзя; для закрытого справочника — оптимально.
2. **Переносы строк (LF/CRLF)** — сборка на Windows конвертировала файлы в CRLF, диффы становились нечитаемыми. Конвенция LF — часть контракта репо (модуль 25).
3. **Битые ссылки между модулями** — при массовых ревизиях нумерация секций менялась, анкоры устаревали. Решение — контрактная проверка ссылок в сборке, а не «глазами».

### Вывод, противоречащий интуиции

SSG-фреймворк не обязателен даже для 52 «страниц»: граница «build vs runtime» (спектр из §1) была решена без фреймворка — статические файлы + собственный сборщик на 200 строк. Фреймворк окупается SEO, персонализацией, ISR-свежестью — всего того, что закрытой документации не нужно.

**Практический вывод для архитектора:** сначала ответь на вопросы из §1 (кто читает, нужна ли свежесть, нужен ли SEO), потом выбирай инструмент. «52 страницы без фреймворка» — не отсталость, а минимизация зависимости: ноль деплой-рисков, ноль обновлений фреймворка, ноль bundle-сюрпризов.

### Дополнение из практики: единый JSON как источник правды (сентябрь 2026)

Следующая ступень того же подхода — генератор, где один JSON описывает структуру, контент, тему, медиа и разметку: PHP для разработки с инклудами для удобной отладки, на выход — чистый HTML без фреймворков. Деплой — на любой хостинг без ноды и баз. Правило: операторский контур отдельно, публикация отдельно — после модерации источником правды остаётся файл данных сайта, а не заявка. Так связываются SSG, пайплайн картинок и почтовый контур без единой базы.
...
Формула: Next.js App Router + revalidate/tags + generateStaticParams (топ-N) + dynamicParams + on-demand route + запрет персональных данных.
