...
### Дополнение из практики: типозависимые правила и формы из схем (сентябрь 2026)

Справочник контрагентов: правила зависят от вида — длина ИНН, обязательность КПП и ОГРН только у юрлиц, паспорт и СНИЛС только у физлиц и ИП. Правила жили только на создании, обновление их не вызывало — мусор сохранялся молча. Фикс — общая функция схемы, подключённая и к созданию, и к обновлению через перекрёстную проверку с разбором эффективного вида, пустое значение считается очисткой, ошибка возвращается как пара поле-текст чтобы фронт подсветил ввод.

Три правила отсюда: правила валидации живут в одном месте и вызываются со всех входов, иначе создание и обновление разъезжаются; частичное обновление проверяется на эффективном объекте, а не на дельте; фронт валидирует до отправки тем же смыслом, бэк — всё равно перепроверяет.

Второй приём — формы из схем: сотни полей карточки описываются конфигурацией, поле расширяется JSONB плюс схема без миграции. Схема здесь — не только контракт API, но и источник формы: какие поля показать, какие обязательны, какие заблокированы по виду. Новая сущность — новая конфигурация, а не новый код формы.
