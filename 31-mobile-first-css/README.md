...
## 7. Реальный кейс: сайт услуг — мобильный first-class

### Контекст

Корпоративный сайт услуг (PHP-шаблоны, модуль 30): страницы услуг, статьи, формы заявок. Трафик на мобильных — ~60% (промышленные заказчики листают с телефона). Вёрстка — desktop-first с одним `@media (max-width: 480px)`: антипаттерн №1 в чистом виде.

### Задача

Мобильный опыт — не «урезанный desktop», а первый класс: карточки услуг, формы, навигация должны работать на 390px. Блоки разного содержания (услуга, статья, форма) — вёрстка не по viewport, а по доступному месту.

### Гипотеза

Mobile-first cascade (базовый layout под мобильный, `min-width`-расширения) + container queries для переиспользуемых блоков: карточка услуги адаптируется к месту (список/боковая колонка), а не к экрану.
...
.service-card__title { font-size: clamp(1rem, 0.9rem + 1vw, 1.25rem); }

/* Расширение по контейнеру, а не по экрану */
@container (min-width: 420px) {
  .service-card { display: grid; grid-template-columns: 72px 1fr; }
  .service-card__icon { display: block; }
}
@container (min-width: 640px) {
  .service-card { display: grid; grid-template-columns: 96px 1fr auto; }
  .service-card__cta { display: inline-block; }
}
```

- Карточки услуг: в списке (широкий контейнер) — с иконкой и CTA; в боковой колонке — компактные. Одна разметка, ноль медиа-правил под контекст;
...
1. **Desktop-first legacy в контенте**: колонка услуг с фиксированной шириной 300px «на мобильном» сжималась до нечитаемой. Базовый layout переписан с нуля — патчи в media-запросах не помогали.
2. **`100dvh`-шапка**: на мобильном адресная строка схлопывалась/раскрывалась — шапка прыгала, CTA-кнопка уезжала. `100svh` для стабильных элементов (антипаттерн №4).
3. **Форма заявки в боковой колонке**: инпуты в узкой колонке были «карликами» — `min-width` на инпутах внутри CQ-блока, а не фикс на весь сайт.

### Вывод, противоречащий интуиции

Переход на mobile-first дал не «лучше на мобильном», а **меньше CSS**: базовый layout один, расширения по контейнерам, никаких каскадов `max-width`-переопределений. Desktop-first вёрстка требовала больше правил, чем мобильно-первая — патчи «скрыть/переложить» умножались с каждым новым блоком.
...
### Дополнение из практики: токены как контракт (август–сентябрь 2026)

Поверх mobile-first лежит слой токенов: всё на CSS-переменных, смена темы — переопределение переменных, шкала отступов и радиусов числом, темы и шрифт и масштаб — атрибутами на html с выбором в локальном хранилище и init-скриптом в head до отрисовки без мигания. Эталон — дюжина палитр без экстрима с контрастом под стандарт доступности и одиннадцать шрифтов с self-host файлами и кириллицей. Грабли из витрины: табличный грид ставит свой шрифт с важным флагом на ячейки — переопределять надо корень и переменную грида с ремаунтом по ключу. Правило: имена переменных не менять, только значения — тогда компоненты переносятся копированием.
