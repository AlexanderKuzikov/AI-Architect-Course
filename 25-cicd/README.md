...
## 7. Реальный кейс: CI для курса из 52 модулей

### Контекст

AI-Architect-Course: 52 модуля в Markdown, сборка в один HTML (course.html, ~5.9 MB) через `node glossary-links.mjs && node build-html.mjs`. Контент меняется массово (ревизии по 10+ файлов за раз), регрессии — битые ссылки между модулями, артефакты `[^N]`/`[web:NNN]` в код-блоках, сломанные code fences.

### Задача

Не пускать регрессии в main: проверять сборку и целостность ссылок при каждом изменении, а не «на глаз» после ревизии.

### Гипотеза

Самый простой CI, который вообще возможен: checkout → setup-node с кэшем → `npm ci` → сборка → fail при ошибке. Сборка — единственная точка правды: если модуль сломан, `build-html.mjs` упадёт.
...
jobs:
  build:
    ...
    steps:
      ...
      - run: node build-html.mjs       # сломанные модули → fail
```

- Массовые ревизии (50+ файлов) стали безопасными: битый модуль ловится за минуту, а не «когда кто-то откроет страницу»;
- `glossary-links.mjs` — фактически контрактный тест: каждая ссылка на модуль проверяется, битая ссылка = красный билд;
- кэш `npm` на `hashFiles('**/package-lock.json')` сократил установку до секунд — CI не тормозит работу.

### Грабли, найденные в production

1. **CRLF ломал сборку**: правки на Windows иногда конвертировали файлы в CRLF — `build-html.mjs` работал, но диффы становились нечитаемыми (весь файл). Фикс — в конвенции репо: LF обязателен, проверка переносов — часть ревизии.
2. **Сломанный код-блок в Markdown не роняет сборку**: непарные ` ``` ` проглатывались markdown-it, но ломали HTML. Контроль количества fences — отдельная проверка в скрипте сборки.
3. **`course.html` в .gitignore**: генерируемый артефакт не коммитится — CI собирает и проверяет, но не публикует артефакт вручную: Pages/релиз берут свежую сборку из CI.

### Вывод, противоречащий интуиции

CI для **контента** (не кода) окупился быстрее, чем для кода: контент меняется массово и редко тестируется «рантаймом» — ошибка живёт месяцами до первого читателя. Сборка + проверка ссылок превратила «контент с ошибками» в «контент с контрактом качества». Для AI-контента (промпты, курсы, документация, генерируемые HTML) — CI это главный детектор регрессий.

**Практический вывод для архитектора:** если контент собирается скриптом — у тебя уже есть CI-точка правды. Минимальный workflow (сборка + проверка ссылок + fail) дешевле любой системы ревью и ловит класс ошибок, который ревью не видит.

### Дополнение из практики: деплой витрины архивом за минуту (август–сентябрь 2026)

Витрина для заказчика выкладывается не через CI, а скриптом: упаковать без тяжёлого мусора, перекинуть по ssh, распаковать, поставить зависимости без изменений lock-файла, проверить типы, собрать, прогнать миграции, перезапустить сервис, перекинуть фронт в корень веб-сервера, перезагрузить прокси. От запуска до ответа по https — минута-полторы.
...
### Дополнение из практики: конвенции как код и онбординг второго (сентябрь 2026)

Стиль коммитов — хуком, а не просьбой: хук режет точки в конце и пустые сообщения, предупреждает о немых «обновлениях» и длине сверх семидесяти двух. Веб-правки хук не ловит — там конвенция руками. Переписывание сотни старых сообщений — только переформулировкой с деревьями один в один, свежий клон после силового пуша обязателен, делает его человек, а не агент.

Онбординг второго разработчика — скриптом и чек-листом: свежий клон, хук в хуки, боевое окружение с носителя без перегенерации ключа шифрования, контейнеры, дамп через распаковку в базу, установка, сверка миграций, запуск, проверка здоровья. Конфликт портов — только хост-стороной компоуза без коммита правки. Проверка что встал — счётчики сущностей через API, расхождение сначала человеку, не чинить молча. Окружение и дампы в git не тащить никогда.
...
Формула: jobs (lint/typecheck/test/build/deploy) + needs-граф + concurrency + permissions + пининг runner + npm ci.
...
Формула: buildx + login (GHCR) + build-push (теги, условие) + registry cache + permissions.
