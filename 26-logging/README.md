...
**7. Логировать чувствительное и секреты**
Чувствительное в логи не печатается никогда: ни телефоны, ни документы, ни шифротекст, ни токены. Проверка здоровья отвечает двумя состояниями без утечек деталей. Сериализаторы — с белым списком полей, а не чёрным: новое чувствительное поле по умолчанию не логируется.

---

## 7. Реальный кейс: request_id против «у нас всё молчит»

### Контекст

CourtDesk — сбор судебных дел + API для 1С-клиента (модуль 51): ~500 запросов/день от регламентного задания, лог — Pino в stdout, каждая запись с `request_id` (генерируется на входе, возвращается клиенту в заголовке `x-request-id` и в теле ошибок).

### Задача

1С-клиент пришёл с тикетом «у нас всё молчит»: выгрузка дел встала, конкретных ошибок нет. Интеграция — регламентное задание, лог на стороне клиента пустой (задание молча завершилось по таймауту).

### Гипотеза

По `request_id` из контракта ошибок (модуль 51) можно найти точную цепочку логов по запросу и восстановить, что произошло на сервере.

### Что получилось

По коду ошибки из тикета (`CASE_SYNC_TIMEOUT`) и `request_id` нашлись логи конкретного запроса:

```text
[2026-08-06T09:14:22.001Z] WARN  req=8f4c-9a2b  POST /v1/import/cases status=429 retry-after=60
[2026-08-06T09:14:23.110Z] WARN  req=8f4c-9a2b  POST /v1/import/cases status=429 retry-after=60
[2026-08-06T09:14:24.003Z] WARN  req=8f4c-9a2b  POST /v1/import/cases status=429 retry-after=60
```

- 1С-клиент ретраил через 1 секунду, игнорируя `Retry-After: 60` — сервер честно лимитил, клиент не читал заголовок;
- одна строка лога (`retry-after`) объяснила часовое «всё молчит»: задание упёрлось в лимит и молча вышло по таймауту;
- фикс — не на сервере: клиент научился уважать `Retry-After` (и поставил backoff с джиттером, см. модуль 52).

### Вывод, противоречащий интуиции

Проблема нашлась за 10 минут — но только потому, что `request_id` был в контракте (модуль 51) и лог был структурированным. **Без `request_id` тикет «всё молчит» неразрешим**: миллион записей от параллельных запросов не связать с жалобой клиента. Наблюдаемость — не «плюс к мониторингу», а часть контракта интеграции: если клиент не может сослаться на конкретный запрос, каждая интеграционная проблема — часы переписки.

**Практический вывод для архитектора:** `request_id` в каждом ответе API + тот же id в логах — минимальная достаточная observability для внешних интеграций. Следующий уровень (traceId, OTel) оправдан внутри сервиса; наружу достаточно одного id.
...
Формула: pino singleton + redact + AsyncLocalStorage (requestId) + child loggers + pino-http + flush при shutdown.
