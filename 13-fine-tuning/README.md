...
**Задача:** fine-tuning для extraction реквизитов из карточек
судебных органов. Базовая модель (актуальная локальная, Q4_K_M) с промптом
давала 72% field accuracy. Нужно 85%+.

**Стек:** Python 3.12, Unsloth 2026.3, QLoRA (r=16, alpha=16),
GTX 1660 6 ГБ, 750 размеченных примеров в Alpaca формате.

**Гипотеза:** fine-tuning на 750 доменных примерах поднимет
точность с 72% до 85%+ без изменения промпта.

**Что получилось:**

Первая попытка — полный датасет 750 примеров, 3 эпохи.
Результат: eval_loss стабильно рос после эпохи 2.

```
Epoch 1: train_loss=1.2, eval_loss=1.4
Epoch 2: train_loss=0.8, eval_loss=1.1
Epoch 3: train_loss=0.4, eval_loss=1.5 ← переобучение!
```

Причина: датасет содержал ~60% лёгких примеров (чистые данные,
однозначные поля). Модель быстро выучила их и начала запоминать,
а не обобщать.

**Решение — стратифицированный датасет:**

```python
# Балансировка: не больше 40% лёгких
dataset = {
    'easy':   examples[:300],   # 300
    'medium': examples[300:600], # 300
    'hard':   examples[600:],    # 150
}
# Oversample medium и hard
sampled = (
    random.choices(dataset['easy'], k=200) +
    dataset['medium'] +
    dataset['hard'] * 2  # дубль hard
)
# Итого: 200 + 300 + 300 = 800
```

Вторая попытка — 800 сбалансированных примеров, 4 эпохи,
early stopping по eval_loss:

```
Epoch 1: train_loss=1.3, eval_loss=1.5
Epoch 2: train_loss=0.9, eval_loss=1.0
Epoch 3: train_loss=0.6, eval_loss=0.8 ← лучше
Epoch 4: train_loss=0.5, eval_loss=0.9 ← стоп (patience=1)
```

Третья попытка увеличить rank до 32 на тех же данных —
eval_loss не улучшился. Rank 16 был достаточен.

**Результат на test set (150 примеров):**

| Метрика | Baseline (prompt only) | Fine-tuned QLoRA r=16 |
|---------|----------------------|----------------------|
| Field accuracy | 72.3% | 87.8% |
| Structural errors (broken JSON) | 8.1% | 0.7% |
| Null rate (false null) | 12.4% | 4.2% |

**Вывод, противоречащий интуиции:**

Увеличение датасета не помогло — помогла **балансировка**
лёгких и сложных примеров. 800 сбалансированных примеров
дали +15.5% accuracy против 750 небалансированных.

Также неожиданно: fine-tuning **почти устранил structural
errors** (broken JSON) даже без изменения схемы. Модель
«запомнила» правильный JSON формат на уровне весов —
constrained decoding не понадобился для базовых случаев
(хотя остался включён для production как safety net).

Cost breakdown:
- Подготовка датасета: ~3 дня (60% времени)
- Обучение: 3 ч 40 мин на GTX 1660
- Экспорт GGUF q4_k_m: ~15 мин
- Дообучение на новых данных (инкрементально): ~40 мин
...
- [ ] Fine-tuning вместо промпта — JSON Schema + system prompt решают проблему формата за 10 минут
- [ ] «Больше эпох = лучше модель» — переобучение на малом датасете наступает на 5-7 эпохе
- [ ] Fine-tune на шумных данных — 500 чистых примеров > 10 000 шумных
- [ ] LoRA rank побольше = качество получше — при малом датасете ведёт к переобучению
- [ ] Деплоить адаптер без merge на production — отдельный LoRA требует PEFT runtime
- [ ] Не фиксировать seed — результаты обучения невоспроизводимы
