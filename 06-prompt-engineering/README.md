...
### Три техники — когда что выбирать

```mermaid
flowchart TD
    subgraph "Decision: какую технику промпта"
        A["Задача"] --> B{"Нужны примеры?"}
        B -->|"Нет"| C["Zero-shot<br/>+ чёткие ограничения"]
        B -->|"Да"| D{"Примеры > 5?"}
        D -->|"Нет"| E["Few-shot<br/>2-5 примеров"]
        D -->|"Да, убывающая отдача"| F["Fine-tuning (Модуль 13)<br/>или RAG (Модуль 12)"]
        C --> G{"Нужно рассуждение?"}
        G -->|"Нет"| H["Прямой ответ<br/>structured output"]
        G -->|"Да"| I["Chain-of-Thought<br/>+ reasoning"]
    end
```
...
### Дополнение из практики: подсказал — человек утвердил (сентябрь 2026)

Операторский контур для карты объектов: модель приводит текст к деловому виду, предлагает категорию из девяти значений и считает количество из описания, если число явно указано. Оператор видит diff свой вариант — предложение модели, правит поля вручную после автозаполнения и только потом отправляет. Вывод для промпта: проси у модели пару suggestion-полей рядом с вычищенным текстом, а не «финальное решение»; решение остаётся за человеком, модель — нормализатор и классификатор. Отдельно: у облачного провайдера думание не отключалось ни флагом, ни подсказкой — сработал только параметр шаблона чата, токены упали в разы; проверяй отключение по счётчикам, а не по факту передачи параметра.
...
- [ ] Добавлять description в enum поля схемы — модель игнорирует, только токены тратит
- [ ] Дублировать схему в system prompt при constrained decoding — два источника правды
- [ ] Использовать один промпт для local-mid и edge SLM — разные возможности
- [ ] `temperature > 0` для extraction pipeline — нестабильный результат, теряешь детерминизм
- [ ] LLM-as-Judge без калибровки против human labels — замена одного bias другим
- [ ] Оптимизировать скорость промпта до измерения качества на репрезентативном датасете
...
*Модуль 06 завершён.*
*Следующий: [Модуль 07 — JSON Schema](../07-json-schema/README.md)*